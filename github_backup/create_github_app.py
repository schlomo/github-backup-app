...
def create_app():
    ...
    try:
        ...
        if data['app_type'] == 'organization':
            ...
            create_url = f"https://github.com/organizations/{data['org_name']}/settings/apps/new"
        else:
...
def print_installation_instructions(app_data: Dict[str, Any], files: Dict[str, str]):
    ...
    print(f"   github-backup --app-id {app_data['id']} --private-key {files['private_key']} --output-directory ./backup --all")
    print()
    print("2. Or backup specific users/organizations:")
    print(f"   github-backup --app-id {app_data['id']} --private-key {files['private_key']} --output-directory ./backup --all USERNAME/ORGANIZATION")
    print()
...
from your OS username. You can change this in the web interface if desired.
    ...
    try:
        ...
        werkzeug.serving.WSGIRequestHandler.log_message = lambda self, format, *args: None
